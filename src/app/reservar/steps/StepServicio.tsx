        ...
        <ul>
          {servicios.map((s, idx) => (
            <li key={s.id} className="relative">
              {idx > 0 && (
                <span className="divider-gold pointer-events-none absolute inset-x-6 top-0" />
              )}
              <button
                type="button"
                onClick={() => onSelect(s.id)}
                className="group relative flex w-full items-center justify-between gap-4 px-6 py-[26px] text-left transition-colors duration-300 hover:bg-gold/[0.06]"
              >
                ...
                <div className="min-w-0 flex-1">
                  <h2 className="display-serif text-[26px] leading-[1.04] text-white drop-shadow-[0_1px_10px_rgba(0,0,0,0.7)] transition-colors duration-300 group-hover:text-gold">
                    {s.nombre}
                  </h2>
                  <p className="mt-2 text-[10px] font-semibold uppercase tracking-[0.34em] text-white/55 transition-colors duration-300 group-hover:text-gold/85">
                    {s.duracion} min
                  </p>
                </div>
                <div className="flex items-center gap-4">
                  <span className="font-serif text-[34px] leading-none tracking-tight text-white drop-shadow-[0_1px_10px_rgba(0,0,0,0.7)] transition-all duration-300 group-hover:text-gold group-hover:drop-shadow-[0_0_22px_rgba(196,164,98,0.45)]">
                    ...
                    <span className="ml-0.5 text-[22px] font-light text-gold/85">€</span>
                  </span>
